...
if [[ -n "$FPM_SERVICE" ]]; then
	echo "Stopping PHP-FPM service: $FPM_SERVICE..."
	...
	echo "✅ Restarted $FPM_SERVICE to refresh pool configuration."
fi
echo ""
...
if command -v supervisorctl >/dev/null 2>&1; then
	echo "🧹 Removing Supervisor config for '$USERNAME'..."

	SUPERVISOR_SYMLINK="/etc/supervisor/conf.d/${USERNAME}_queue.conf"
	USER_SUPERVISOR_DIR="/home/$USERNAME/supervisor"
	...
	if pgrep -f supervisord >/dev/null; then
		...
		echo "Removing Supervisor symlink: ${SUPERVISOR_SYMLINK}..."
		sudo rm -f "$SUPERVISOR_SYMLINK"
		...
		echo "Deleting user Supervisor config and logs..."
		rm -rf "$USER_SUPERVISOR_DIR"
		rm -f "$USER_LOG_DIR/queue_worker.log"

		echo "✅ Supervisor cleanup complete for '$USERNAME'."
	else
